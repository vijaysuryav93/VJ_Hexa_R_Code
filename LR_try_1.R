...
model = glm(Hexa_Markup ~ ., family = binomial(link = 'logit'), data = lg_train_df)
summary(model)
fitted_prob <- predict(model, lg_test_df, type = 'response')
...
lg_test_df[fitted_prob<0.5, "fitted_prob"] <- "Not"
lg_test_df[fitted_prob>=0.5, "fitted_prob"] <- "Target"
#View(lg_test_df)
#str(lg_test_df)
#dim(lg_test_df)
...
#View(lg_data$X)
dim(lg_data)
surge_file_details$fitted_prob <- lg_data$fitted_prob[match(surge_file_details$Account.Domain,lg_data$X)]
surge_file_Target_Accounts <- surge_file_details %>% filter( surge_file_details$fitted_prob == "Target")
...
library(caret)
(table(surge_file_details$Hexa_Markup, surge_file_details$fitted_prob))
